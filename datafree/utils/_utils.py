...
def normalize(tensor, mean, std, reverse=False):
    ...
    # 将 mean 和 std 转换为张量
    _mean = paddle.to_tensor(_mean, dtype=tensor.dtype, place=tensor.place)
    _std = paddle.to_tensor(_std, dtype=tensor.dtype, place=tensor.place)

    # 执行归一化操作
    tensor = (tensor - _mean.reshape([1, -1, 1, 1])) / _std.reshape([1, -1, 1, 1])
    return tensor
...
def save_image_batch_labeled(imgs, targets, batch_dir, batch_id=None, col=None, size=None, pack=True):
    if isinstance(imgs, paddle.Tensor):
        imgs = (paddle.clip(imgs.detach(), 0, 1).cpu().numpy() * 255).astype('uint8')  # 将张量转为numpy格式，并缩放到[0, 255]范围
    ...
    if not pack:
        ...
        for idx, img in enumerate(imgs):
            ...
            if img.shape[0] == 1:  # 如果是灰度图
                img = Image.fromarray(img[0])
            else:  # RGB 图像
                img = Image.fromarray(img.transpose(1, 2, 0))  # 转置维度
...
def dense_kldiv(logits, targets, T=1.0, reduction='batchmean'):
    # 计算经过温度缩放的 log_softmax 和 softmax
    q = F.log_softmax(logits / T, axis=1)
    p = F.softmax(targets / T, axis=1)
    
    # 计算 KL 散度
    kl_div = F.kl_div(q, p, reduction=reduction) * (T * T)
