...
class LocalUpdate(object):
    ...
    def update_weights(self, model, client_id):
        model.train()
        optimizer = paddle.optimizer.Momentum(parameters=model.parameters(), learning_rate=self.args.local_lr, momentum=0.9)
        local_acc_list = []
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.train_loader):
                ...
                acc += paddle.sum(paddle.argmax(output, axis=1) == labels).item()
                total_num += len(labels)
                train_loss += loss.item()
                # ---------------------------------------
                loss.backward()
                paddle.nn.utils.clip_grad_norm_(parameters=model.parameters(), max_norm=max_norm)
                optimizer.step()
            train_loss = train_loss / total_num
            acc = acc / total_num
            self.logger.info('Iter:{:4d} Train_set: Average loss: {:.4f}, Accuracy: {:.4f}'.format(iter, train_loss, acc))
...
def args_parser():
    ...
    parser.add_argument('--beta', default=0.5, type=float, help=' If beta is set to a smaller value, then the partition is more unbalanced')
    parser.add_argument('--sigma', default=0.0, type=float, help=' If beta is set to a smaller value, then the partition is more unbalanced')
    # Default
    parser.add_argument('--seed', default=42, type=int, help='seed for initializing training.')
    parser.add_argument('--model', default="", type=str, help='models for each client.')
    parser.add_argument('--identity', default="", type=str, help='identity.')
    parser.add_argument('--logidentity', default="", type=str, help='logidentity.')
...
if __name__ == '__main__':
    ...
    logpath_prefix = '/home/dairong/coboost/LOG/FL_pretrain/'
    ...
    num_classes, train_dataset, test_dataset = registry.get_dataset(name=args.dataset, data_root='/home/dairong/coboost/Data/mnist')
    ...
    test_loader = paddle.io.DataLoader(test_dataset, batch_size=256, shuffle=False, num_workers=4)
    # Build models
    global_model = registry.get_model(args.model, num_classes=num_classes)
    global_model = global_model.to('gpu')
    ...
    # Save models
    save_path_prefix = '/home/dairong/coboost/checkpoints/FL_pretrain/'
    if not os.path.exists(save_path_prefix):
        os.makedirs(save_path_prefix)
    paddle.save(local_weights, save_path_prefix + '{}.pkl'.format(args.identity))

    # Test FedAvg model
    global_model = registry.get_model(args.model, num_classes=num_classes)
    global_model = global_model.to('gpu')
    global_weights = average_weights(local_weights)
    global_model.set_dict(global_weights)
    ...
    logger.info('FedAvg global model acc: Average loss: {:.4f}, Accuracy: {:.4f}'.format(test_loss, test_acc))
    ...
    logger.info('Direct Ensemble model acc: Average loss: {:.4f}, Accuracy: {:.4f}'.format(test_loss, test_acc))
